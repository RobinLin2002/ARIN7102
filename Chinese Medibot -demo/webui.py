...
def load_model(cache_model):
    ...
    bert_model.load_state_dict(torch.load(f'model/{cache_model}.pt', map_location='cuda:0'))  # 或
...
def main(is_admin, usname):
    ...
    st.title(f"Medical AI dialogue Robot")
    ...
    with st.sidebar:
        ...
        st.caption(
            f"""<p align="left">Welcom，{'Administrator' if is_admin else 'User'}{usname}! This is KED：{1.0}</p>""",
        ...
        window_options = [f"window {i + 1}" for i in range(len(st.session_state.chat_windows))]
        selected_window = st.selectbox('Please choose conversation windows:', window_options)
        active_window_index = int(selected_window.split()[1]) - 1

        selected_option = st.selectbox(
            label='Choose LLM:',
            options=['Qwen 1.5', 'Finetuned Qwen 1.5']
        )
        choice = 'qwen:7b' if selected_option == 'Finetuned Qwen 1.5' else 'deepseek-r1:1.5b'

        show_ent = show_int = show_prompt = False
        # if is_admin:
        show_ent = st.sidebar.checkbox("Show Name Entity")
        show_int = st.sidebar.checkbox("Show User Intent")
    ...
    if query := st.chat_input("Ask me anything!", key=f"chat_input_{active_window_index}"):
        ...
        response_placeholder.text("Recognitioning User's Intent...")
